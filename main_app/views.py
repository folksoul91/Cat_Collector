...
def add_photo(request, cat_id):
  ...
  # use conditional logic to determine if file is present (s3)isbuiltin
  if photo_file:
  # if it's present, we will create a reference to the boto3 client
    ...
    # if successful
      ...
      url = f"{S3_BASE_URL}{BUCKET}/{key}"
        #1) create photo instance w/ photo model and provide cat_id as foreign key value
  ...
  we need to create a new user b/c form was submitted
  ...
  2)validate the form to ensure it was completed
    2a) if form not valid- redirect the user to the signup page w/ an error message
  3) saving the user obj to the database
